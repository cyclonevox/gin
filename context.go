...
func (c *Context) Bind(obj any) error {
	c.Request.Form = make(map[string][]string, len(c.Params))
...
func bind(req *http.Request, obj interface{}) error {
	...
	// 绑定json格式的请求体
	if err = json.NewDecoder(req.Body).Decode(obj); err != nil && !errors.Is(err, io.EOF) {
		return err
	}

	// 绑定query参数
	if err = bindData(obj, req.URL.Query(), "query"); err != nil {
		return err
	}

	// 绑定header参数
	if err = bindData(obj, req.Header, "header"); err != nil {
		return err
	}

	// 绑定param参数
	if err = bindData(obj, req.Form, "param"); err != nil {
		return err
	}
